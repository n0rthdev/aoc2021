...
function fun120801() {
    ...
    let input = getInput1208().split('\n').map(line => line.trim().split(' | ').map(part => part.split(' ')));
...
function fun120802() {
    ...
    for(let i = 0; i < input.length;i++)
    {
       console.log("line " + i + ": " + input[i].toString())
       counter += doline(input[i])
    }

    console.log("count: " + counter)
}

function doline(line : string[][]) : number
{    
    let allDigits = line[0].map(it => it.split('').sort())
    let identified = Array<Array<string>>(10);
    identified[1] = allDigits.filter(it => it.length == 2)[0];
    identified[7] = allDigits.filter(it => it.length == 3)[0];
    identified[4] = allDigits.filter(it => it.length == 4)[0];
    identified[8] = allDigits.filter(it => it.length == 7)[0];

    let g690 = allDigits.filter(it => it.length == 6)

    identified[6] = g690.filter(it => it.indexOf(identified[1][0]) == -1 || it.indexOf(identified[1][1]) == -1)[0];

    let g235 = allDigits.filter(it => it.length == 5)

    identified[5] = g235.filter(it => it.filter(it => identified[6].indexOf(it) >= 0).length == 5)[0];

    let e = identified[6].filter(it => identified[5].indexOf(it) == -1)[0];

    identified[0] = g690.filter(it => it != identified[6]).filter(it => it.indexOf(e) != -1)[0]

    identified[9] = g690.filter(it => it != identified[6]).filter(it => it != identified[0])[0]

    identified[2] = g235.filter(it => it != identified[5]).filter(it => it.indexOf(e) != -1)[0]

    identified[3] = g235.filter(it => it != identified[5]).filter(it => it != identified[2])[0]

    let targetDigits = line[1].map(it => it.split('').sort())

    let mymap = new Map<string,number>()

    for(let i = 0; i < 10;i++)
    {
        mymap.set(identified[i].toString(),i)
    }

    let target = targetDigits.map(it => mymap.get(it.toString()))

    return parseInt(target.join(''))
}
...
function getTest1208a(): string {
    return `fgeab ca afcebg bdacfeg cfaedg gcfdb baec bfadeg bafgc acf | gebdcfa ecba ca fadegcb`;
}

function getTest1208(): string {
...
function getInput1208(): string {
    return `dcagfb cbegda cabdf bagedf dcb cd dcbegaf aebcf bgdaf dgcf | dacebgf cbafd dbacf cdgf
